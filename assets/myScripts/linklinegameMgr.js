...
cc.Class({
    ...
    // 广告 游戏中心 iap 初始化
    linklineinitAD_gameCenter_iap:function(){
        myADMgr.init();
    ...
    start:function(){
        ...
        this.removeAd_btn.getChildByName('off001').runAction(cc.repeatForever(cc.sequence(
            cc.rotateTo(0.15,-30),
            cc.rotateTo(0.15,30),
            cc.rotateTo(0.075,0),
            cc.delayTime(1)
    ...
    // 界面淡入
    linklinemyFadeIn:function(node){
        node.opacity = 0;
        node.setPosition(0, 0);
        node.runAction(cc.fadeIn(0.3));
    },
    // 界面淡出
    linklinemyFadeOut:function(node, pos){
        var myCallFunc = cc.callFunc(function(){   // callFunc 要写前面
            node.setPosition(pos);
        });
        node.runAction(cc.sequence(cc.fadeOut(0.3), myCallFunc));
    },

    linklinemyMoveLeft(node){
        node.runAction(cc.moveBy(0.3,cc.v2(-852,0)));
    },

    linklinemyMoveRight(node){
    ...
    linklinemySlideOut:function(node){
        ...
        node.runAction(cc.sequence(
            cc.moveTo(0.3, cc.v2(0, -1000)).easing(cc.easeBackIn()),
    ...
    linklinestartGameBtn:function(){
        ...
        this.scheduleOnce(function(){
            ...
            if(this.isFirst){
                ...
                this.scheduleOnce(function(){
                    let js = this.gameNode.getComponent("linklinegame");
                    js.linklinechunkAct();
                }, 0.3);
            }
        },0.2)
    ...
    // 排行榜按钮
    linklinerankListBtn:function(){
        this.audioMgr.linklineplayUi_btn();
        myGameCenterMgr.openGameCenter();
    },
    // 去广告
    linklineremoveAdBtn:function(){
        this.audioMgr.linklineplayUi_btn();
    ...
    // 死亡界面按钮
    // 返回游戏菜单
    linklinereturnMenu_gameover:function(){
        this.audioMgr.linklineplayUi_btn();
        cc.find('Canvas/block').active = true;
        this.scheduleOnce(function(){
            cc.find('Canvas/block').active = false;
    ...
    // 重新开始
    linklineagain_gameover:function(){
        this.audioMgr.linklineplayUi_btn();
        cc.find('Canvas/block').active = true;
        this.scheduleOnce(function(){
            cc.find('Canvas/block').active = false;
    ...
    linklineskinStore(){
        this.audioMgr.linklineplayUi_btn();
        let node = cc.find('Canvas/gameMenu/gameSkin')
        node.active = true;
    ...
    linklinetoStatistic(){
        this.audioMgr.linklineplayUi_btn();
        let node = cc.find('Canvas/gameMenu/statistics')
        node.active = true;
